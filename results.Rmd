...
Notably, the prior for each effects $\beta_{l}$ set as the Laplace distribution such that the model drive the effects to zero. Finally, the hyper-priors are set as the flat, non-informative priors mostly, since we do not have any prior knowledge about the data distribution.
...
We run a few diagnostics for the model convergence. It seems we need to run model longer run (at least 5~6 times) according to Gelman's diganostics. The auto-correlation diagnostics also suggests it needs more long run. We suppose it's mostly due to the nested structure we introduced to the experiment is not having strong grouping effect. But to ensure this we might need a further study.
...
- `audio`
- `value` (it's almost also sure for 95% level, but it's not as always dodging it, but also our model is not fully converged yet.)
- `liwc`
- `liwc:task[genre-clf]`